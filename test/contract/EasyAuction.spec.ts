...
describe("EasyAuction", async () => {
  ...
  describe("initiate Auction", async () => {
    ...
    it.skip("throws if auctioned amount is zero", async () => {
      const {
        auctioningToken,
        biddingToken,
      } = await createTokensAndMintAndApprove(
      ...
      // bcoz cd3d token checks amt > 0 and auction transfers 1st then checks amt require > 0
      await expect(
        createAuctionWithDefaults(easyAuction, {
          auctioningToken,
          biddingToken,
    ...
    it("initiateAuction stores the parameters correctly", async () => {
      ...
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("1", "9"),
        buyAmount: ethers.utils.parseEther("0.0000000001"),
      ...
      await expect(auctionData.clearingPriceOrder).to.equal(
        encodeOrder({
          ...
          sellAmount: ethers.utils.parseUnits("0", "9"),
  ...
  describe("placeOrdersOnBehalf", async () => {
    it("places a new order and checks that tokens were transferred", async () => {
      ...
      const buyAmount = ethers.utils.parseEther("0.000000001");
  ...
  describe("placeOrders", async () => {
    ...
    it("does not withdraw funds, if orders are placed twice", async () => {
      ...
      await expect(() =>
        easyAuction.placeSellOrders(
          auctionId,
          [ethers.utils.parseUnits("1", "9").sub(1)],
    ...
    it("places a new order and checks that tokens were transferred", async () => {
      ...
      const sellAmount = ethers.utils.parseUnits("1", "9").add(1);
      const buyAmount = ethers.utils.parseEther("0.000000001");
    ...
    it("throws, if DDOS attack with small order amounts is started", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("1", "9").div(5000),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(10000),
          userId: BigNumber.from(1),
        },
      ];
  ...
  describe("precalculateSellAmountSum", async () => {
    it("fails if too many orders are considered", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("1", "9").mul(2),
          buyAmount: ethers.utils.parseEther("0.00000001").div(5),
          userId: BigNumber.from(1),
        },
        {
          sellAmount: ethers.utils.parseUnits("1", "9").mul(2),
          buyAmount: ethers.utils.parseEther("0.00000001"),
          userId: BigNumber.from(1),
        },

        {
          sellAmount: ethers.utils.parseUnits("1", "9").mul(2).add(1),
          buyAmount: ethers.utils.parseEther("0.00000001").mul(2),
          userId: BigNumber.from(1),
        },
      ];
      const {
    ...
    it("fails if queue end is reached", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("1", "9").mul(2),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(5),
          userId: BigNumber.from(1),
        },
      ];
      const {
    ...
    it("verifies that interimSumBidAmount and iterOrder is set correctly", async () => {
      ...
      const sellOrders = [
        ...
        {
          sellAmount: ethers.utils.parseUnits("1", "9").mul(2).add(1),
          buyAmount: ethers.utils.parseEther("0.0000000001").mul(2),
          userId: BigNumber.from(1),
        },
      ];
      const {
    ...
    it("verifies that interimSumBidAmount and iterOrder takes correct starting values by applying twice", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("1", "9"),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(10),
          userId: BigNumber.from(1),
        },
        {
          sellAmount: ethers.utils.parseUnits("1", "9"),
  ...
  describe("settleAuction", async () => {
    it("checks case 4, it verifies the price in case of clearing order == initialAuctionOrder", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("1", "9").div(10),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(20),
          userId: BigNumber.from(1),
        },
      ];
    ...
    it("checks case 4, it verifies the price in case of clearingOrder == initialAuctionOrder", async () => {
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("5", "9"),
        buyAmount: ethers.utils.parseEther("0.0000000001"),
        userId: BigNumber.from(1),
      };
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("0.1", "9"),
          buyAmount: ethers.utils.parseEther("0.0000000001"),
          userId: BigNumber.from(1),
        },
      ];
    ...
    it("checks case 4, it verifies the price in case of clearingOrder == initialAuctionOrder with 3 orders", async () => {
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("2", "9"),
        buyAmount: ethers.utils.parseEther("0.0000000001"),
        userId: BigNumber.from(1),
      };
      const sellOrders = [
    ...
    it("checks case 6, it verifies the price in case of clearingOrder == initialOrder, although last iterOrder would also be possible", async () => {
      ...
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("500", "9"),
        buyAmount: ethers.utils.parseEther("0.0000000001"),
        userId: BigNumber.from(1),
      };
      const sellOrders = [
    ...
    it("checks case 12, it verifies that price can not be the initial auction price (Adam's case)", async () => {
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("1", "9").add(1),
        buyAmount: ethers.utils.parseEther("0.0000000001"),
        userId: BigNumber.from(1),
      };
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("0.5", "9"),
          buyAmount: ethers.utils.parseEther("0.0000000001"),
          userId: BigNumber.from(1),
        },
        {
    ...
    it("checks case 3, it verifies the price in case of clearingOrder != placed order with 3x participation", async () => {
      ...
      expect(auctionData.clearingPriceOrder).to.equal(
        encodeOrder({
          sellAmount: ethers.utils.parseUnits("3", "9"),
    ...
    it("checks case 2, it verifies the price in case without a partially filled order", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("1", "9").mul(2),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(2),
          userId: BigNumber.from(1),
        },
        {
          sellAmount: ethers.utils.parseUnits("1", "9").add(1),
    ...
    it("checks case 10, verifies the price in case one sellOrder is eating initialAuctionOrder completely", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("1", "9").mul(20),
          buyAmount: ethers.utils.parseEther("0.0000000001").mul(10),
          userId: BigNumber.from(1),
        },
      ];
      const {
    ...
    it("checks case 5, bidding amount matches min buyAmount of initialOrder perfectly", async () => {
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("1", "9"),
        buyAmount: ethers.utils.parseEther("0.00000000005"),
        userId: BigNumber.from(1),
      };
      const sellOrders = [
        {
    ...
    it("checks case 7, bidding amount matches min buyAmount of initialOrder perfectly with additional order", async () => {
      ...
      const sellOrders = [
        ...
        {
          sellAmount: ethers.utils.parseUnits("1", "9"),
          buyAmount: ethers.utils.parseEther("0.00000000006"),
          userId: BigNumber.from(3),
        },
      ];
      const {
    ...
    it("checks case 10: it shows an example why userId should always be given: 2 orders with the same price", async () => {
      ...
      const sellOrders = [
        ...
        {
          sellAmount: ethers.utils.parseUnits("0.5", "9"),
          buyAmount: ethers.utils.parseEther("0.0000000004"),
          userId: BigNumber.from(3),
        },
      ];
      const {
    ...
    it("simple version of e2e gas test", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("1", "9").div(4),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(8),
          userId: BigNumber.from(1),
        },
        {
          sellAmount: ethers.utils.parseUnits("1", "9").div(4),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(12),
          userId: BigNumber.from(1),
        },
        {
          sellAmount: ethers.utils.parseUnits("1", "9").div(4),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(16),
          userId: BigNumber.from(1),
        },
        {
          sellAmount: ethers.utils.parseUnits("1", "9").div(4),
    ...
    it("checks whether the minimalFundingThreshold is not met", async () => {
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("10", "9"),
        buyAmount: ethers.utils.parseEther("0.00000001"),
        userId: BigNumber.from(1),
      };
      const sellOrders = [
        ...
        {
          sellAmount: ethers.utils.parseUnits("1", "9"),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(4),
          userId: BigNumber.from(1),
        },
      ];
      const {
  ...
  describe("claimFromAuctioneerOrder", async () => {
    ...
    it("checks the claimed amounts for a partially matched initialAuctionOrder and buyOrder", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("2", "9").div(2).add(1),
          buyAmount: ethers.utils.parseEther("0.000000001").div(2),
          userId: BigNumber.from(1),
        },
      ];
      const {
  ...
  describe("claimFromParticipantOrder", async () => {
    ...
    it("checks the claimed amounts for a partially matched buyOrder", async () => {
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("100", "9"),
        buyAmount: ethers.utils.parseEther("0.0000000001"),
        userId: BigNumber.from(1),
      };
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("1", "9").div(2).add(1),
          buyAmount: ethers.utils.parseEther("0.0000000001").div(2),
          userId: BigNumber.from(1),
        },
        {
          sellAmount: ethers.utils.parseUnits("1", "9").mul(2).div(3).add(1),
          buyAmount: ethers.utils.parseEther("0.0000000001").mul(2).div(3),
          userId: BigNumber.from(1),
        },
      ];
      const {
    ...
    it("checks the claimed amounts for a fully not-matched buyOrder", async () => {
      ...
      const sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("10", "9").div(2).add(1),
          buyAmount: ethers.utils.parseEther("0.00000001").div(2),
          userId: BigNumber.from(1),
        },
        {
          sellAmount: ethers.utils.parseUnits("10", "9").mul(2).div(3).add(1),
          buyAmount: ethers.utils.parseEther("0.00000001").mul(2).div(3),
          userId: BigNumber.from(1),
        },
        {
          sellAmount: ethers.utils.parseUnits("10", "9").mul(2).div(3).add(1),
  ...
  describe("settleAuctionAtomically", async () => {
    it("can not settle atomically, if it is not allowed", async () => {
      ...
      const atomicSellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("0.499", "9"),
          buyAmount: ethers.utils.parseEther("0.0000000004999"),
          userId: BigNumber.from(2),
        },
      ];
      const {
    ...
    it("reverts, if more than one order is intended to be settled atomically", async () => {
      ...
      const atomicSellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("0.49", "9"),
          buyAmount: ethers.utils.parseEther("0.00000000049"),
          userId: BigNumber.from(2),
        },
        {
          sellAmount: ethers.utils.parseUnits("0.4", "9"),
          buyAmount: ethers.utils.parseEther("0.0000000004"),
          userId: BigNumber.from(2),
        },
      ];
    ...
    it("can not settle atomically, if precalculateSellAmountSum was used", async () => {
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("10", "9"),
        buyAmount: ethers.utils.parseEther("0.000000005"),
        userId: BigNumber.from(1),
      };
      const sellOrders = [
        {
      ...
      const atomicSellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("4.99", "9"),
          buyAmount: ethers.utils.parseEther("0.00000004999"),
          userId: BigNumber.from(2),
        },
      ];
      const {
    ...
    it("allows an atomic settlement, if the precalculation are not yet beyond the price of the inserted order", async () => {
      ...
      const atomicSellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("0.5", "9"),
          buyAmount: ethers.utils.parseEther("0.000000000055"),
          userId: BigNumber.from(1),
        },
      ];
      const {
    ...
    it("can settle atomically, if it is allowed", async () => {
      ...
      const atomicSellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("0.4999", "9"),
          buyAmount: ethers.utils.parseEther("0.0000000004999"),
          userId: BigNumber.from(2),
        },
      ];
  ...
  describe("claimsFee", async () => {
    ...
    it("claims fees fully for a partially filled initialAuctionOrder", async () => {
      const initialAuctionOrder = {
        sellAmount: ethers.utils.parseUnits("10", "9"),
        buyAmount: ethers.utils.parseEther("0.000000000001"),
        userId: BigNumber.from(1),
      };
      let sellOrders = [
        {
          sellAmount: ethers.utils.parseUnits("10", "9").div(4),
          buyAmount: ethers.utils.parseEther("0.000000000001").div(4).sub(1),
          userId: BigNumber.from(3),
        },
      ];
      const {
